...
exports.list = (req, res) => {
    Blog.find({})
        ...
        .select('_id title slug excerpt categories tags postedBy createdAt updatedAt isPublished')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
...
exports.read = (req, res) => {
    ...
    Blog.findOne({ slug })
        ...
        .populate('tags', '_id name slug')
        .populate('postedBy', '_id name username')
        .select('_id title body slug mtitle mdesc categories tags postedBy createdAt updatedAt isPublished')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
...
exports.listByUser = (req, res) => {
    User.findOne({ username: req.params.username }).exec((err, user) => {
        ...
        Blog.find({ postedBy: userId })
            ...
            .populate('tags', '_id name slug')
            .populate('postedBy', '_id name username')
            .select('_id title slug postedBy createdAt updatedAt isPublished')
            .exec((err, data) => {
                if (err) {
                    return res.status(400).json({
                        error: errorHandler(err)
